...
class HotPepperApi {
  ...
  static const _apiKey = String.fromEnvironment('HOTPEPPER_API_KEY');
  static const _endpoint = 'https://webservice.recruit.co.jp/hotpepper/gourmet/v1/';
  ...
  Future<List<Shop>> search({
    ...
    if (_apiKey.isEmpty) {
      throw const HotPepperApiException(
        'APIキーが設定されていません。HOTPEPPER_API_KEY を dart-define で指定してください。',
      );
    }
    ...
    final response = await _client.get(Uri.parse(_endpoint).replace(queryParameters: parameters));
